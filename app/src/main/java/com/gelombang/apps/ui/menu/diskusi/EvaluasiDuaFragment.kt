...
import com.gelombang.apps.databinding.FragmentEvaluasiDuaBinding
 ...
 */
class EvaluasiDuaFragment : Fragment() {
    companion object {
        const val TYPE = "TYPE"
        const val DATA = "DATA"
        const val NAME = "NAME"
    }
    private var uri = arrayOfNulls<Uri>(6)
    private var images = arrayOfNulls<ImageView>(6)
    ...
    override fun onCreateView(
        ...
        val binding: FragmentEvaluasiDuaBinding = FragmentEvaluasiDuaBinding.inflate(layoutInflater)
        ...
        if (type.equals("guru")) {
            ...
            showDialog(activity!!)
            val pathReference:StorageReference = storageRef!!.child("Diskusi").child("PertemuanDua").child(hasilEvaluasi?.key + "0.jpg")
            pathReference.downloadUrl.addOnSuccessListener { uri ->
                ...
                val pathReference1:StorageReference = storageRef!!.child("Diskusi").child("PertemuanDua").child(hasilEvaluasi?.key + "1.jpg")
                pathReference1.downloadUrl.addOnSuccessListener { uri1 ->
                    ...
                    val pathReference2:StorageReference = storageRef!!.child("Diskusi").child("PertemuanDua").child(hasilEvaluasi?.key + "2.jpg")
                    pathReference2.downloadUrl.addOnSuccessListener { uri2 ->
                        ...
                        val pathReference3:StorageReference = storageRef!!.child("Diskusi").child("PertemuanDua").child(hasilEvaluasi?.key + "3.jpg")
                        pathReference3.downloadUrl.addOnSuccessListener { uri3 ->
                            ...
                            val pathReference4:StorageReference = storageRef!!.child("Diskusi").child("PertemuanDua").child(hasilEvaluasi?.key + "4.jpg")
                            pathReference4.downloadUrl.addOnSuccessListener { uri4 ->
                                ...
                                val pathReference5:StorageReference = storageRef!!.child("Diskusi").child("PertemuanDua").child(hasilEvaluasi?.key + "5.jpg")
    ...
    private fun sendAnswer(
        ...
        showDialog(activity!!)
        val key: String? = mDatabase!!.child("PertemuanDua").push().key
        for (x: Int in imageUri.indices) {
            val path: StorageReference = storageRef!!.child("Diskusi").child("PertemuanDua").child("$key$x.jpg")
            ...
            uploadTask.addOnSuccessListener {
                if (x == imageUri.size - 1) {
                    ...
                    maps["status"] = "0"
                    maps["nilai"] = ""
                    mDatabase!!.child("PertemuanDua").child(key!!).setValue(maps)
